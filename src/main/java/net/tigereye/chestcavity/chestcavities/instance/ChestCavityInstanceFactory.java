...
public class ChestCavityInstanceFactory {
    ...
    public static ChestCavityInstance newChestCavityInstance(EntityType<? extends LivingEntity> entityType, LivingEntity owner) {
        ...
        if (GeneratedChestCavityAssignmentManager.GeneratedChestCavityAssignments.containsKey(entityID)) {
            ResourceLocation chestCavityTypeID = (ResourceLocation)GeneratedChestCavityAssignmentManager.GeneratedChestCavityAssignments.get(entityID);
            if (GeneratedChestCavityTypeManager.GeneratedChestCavityTypes.containsKey(chestCavityTypeID)) {
                return new ChestCavityInstance((ChestCavityType)GeneratedChestCavityTypeManager.GeneratedChestCavityTypes.get(chestCavityTypeID), owner);
            }
        }

        return entityIdentifierMap.containsKey(entityID) ? new ChestCavityInstance((ChestCavityType)entityIdentifierMap.get(ForgeRegistries.ENTITY_TYPES.getKey(entityType)), owner) : new ChestCavityInstance(DEFAULT_CHEST_CAVITY_TYPE, owner);
    }
