...
public class ReceivedChestCavityUpdatePacket {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            ChestCavityEntity.of(context.getSender()).ifPresent(chestCavityEntity ->
                    NetworkUtil.ReadChestCavityReceivedUpdatePacket(
                            chestCavityEntity.getChestCavityInstance()
                    )
            );
        });
